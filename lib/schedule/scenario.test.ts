...
import type { AvailabilityBlock, Item, Weekday } from "@/lib/db/types";
...
function item(t: Partial<Item> & Pick<Item, "title">): Item {
  return {
    id: id("item"),
    user_id: "u1",
    notes: null,
...
function weekly(t: Partial<Item> & Pick<Item, "title">): Item {
  return item({
...
const RECURRING = [
  weekly({
    ...
    confidence: 2,
    priority: 1,
    due_at: localInstant(28, 9, 0).toISOString(), // exam in 11 days
  }),
  weekly({ title: "Linear Algebra", target_minutes_per_week: 120, confidence: 4 }),
  weekly({ title: "Spanish", target_minutes_per_week: 90, confidence: 3 }),
];

/** One list now — the scheduler no longer distinguishes the two. */
const ITEMS = [...FINITE, ...RECURRING];
...
function render(plan: ReturnType<typeof buildPlan>["plan"]): string {
  ...
  for (const block of plan.blocks) {
    ...
    const subject = byId.get(block.item_id);
    const label = subject?.title ?? "?";
    const marker = subject?.target_minutes_per_week === null ? "•" : "○";
    lines.push(
  ...
  if (plan.unplaced.length > 0) {
    ...
    for (const item of plan.unplaced) {
      lines.push(
        `    ${byId.get(item.id)?.title ?? item.label}: ${item.minutesShort}m short — ${item.reason}`,
      );
    }
  }
  return lines.join("\n");
...
describe("a realistic week", () => {
  it("produces a plan a human would accept", () => {
    ...
    for (const block of plan.blocks) {
      const owner = ITEMS.find((entry) => entry.id === block.item_id);
      if (owner?.due_at) {
    ...
    expect(plan.blocks[0].item_id).toBe(
      ITEMS.find((entry) => entry.title === "Finish 6.006 pset 4")!.id,
    );

    // Indivisible work stayed in one piece.
    const passport = ITEMS.find((entry) => entry.title === "Renew passport")!;
    const passportBlocks = plan.blocks.filter((b) => b.item_id === passport.id);
